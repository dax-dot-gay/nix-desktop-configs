    ...
    systemd.services."check-and-notify-update" = {
        ...
        script = ''
            if [ $(echo "$(${comin.packages."x86_64-linux".default} status --json)" | ${pkgs.jq} .deploy_confirmer.submitted) != '""' ]; then
                for id in $(${pkgs.loginctl} list-sessions -j | ${pkgs.jq} -r '.[] | .session') ; do
                    if [[ $(${pkgs.loginctl} show-session $id --property=Type) =~ (wayland|x11) ]] ; then
                        USER=$(${pkgs.loginctl} show-session $id --property=Name --value)
